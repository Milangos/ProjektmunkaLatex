...
\begin{abstract}
This document presents the clustering analysis of Type 1 Diabetes and the associated metabolic simulators. We introduce the py-mGIPsim and DMMS.R systems and provide a comparative overview of their functionalities. Our planned future work includes extending the simulations with snack intake and physica exercise, as well as applying clustering methods to the simulation results to identify common patterns in blood glucose dynamics among patients.
\end{abstract}


\begin{IEEEkeywords}
diabetes, type-1, simulator, metabolic model, DMMS.R, py-mGIPsim, clustering
\end{IEEEkeywords}
...
Managing Type 1 diabetes (T1D) primarily revolves around insulin therapy, which replaces the insulin the body no longer produces. Since T1D is a self-managed condition, patients make daily decisions about their insulin needs to maintain blood sugar levels within a target range.

Insulin comes in various forms, each with different speeds of onset and duration of action. Patients typically require a combination of \cite{treatment_type1}:
...
\item Bolus Insulin (Mealtime and Correction): Specific doses taken when eating (to cover carbohydrate intake) and to correct elevated blood sugar levels.
\end{itemize}
...
\item Insulin Pumps: Devices that deliver insulin continuously through a small tube inserted under the skin, aiming to mimic the natural function of the pancreas. Additional doses can also be delivered on demand.
\end{itemize}

\section{First simulator: py-mGIPsim\cite{py_mgipsim}}
...
\section{Second simulator: DMMS.R\cite{dmmsr}}
...
During the simulation, the virtual patients do not engage in exercise and are prohibited from consuming any snacks, treats, or supplementary foods. They receive only the three meals specified in the protocol:
...
The input data for the simulations consists of the existing datasets provided by the simulators, which are the basal and bolus insulin values calculated individually for each patient based on their specific physiological needs. Modifying these inputs is possible when using mGIPsim, but quite difficult with DMMS.R. These values are highly personalized and vary significantly from person to person, making any corrections or adjustments particularly challenging to compute. This individualized variability is one of the reasons why clustering such data presents considerable difficulties.

\begin{figure}[!ht]
    \centering
    \includegraphics[width=1\linewidth]{gipsimBasalSortedByWeight.png} % replace with your image
    \caption{Graphical summary of input data showing variability among patients. Basal insulin rates over 24 hours for all 20 patients, sorted by body weight. Clearly, there is no direct correlation between body weight and insulin needs.}
    \label{fig:input1}
\end{figure}
The figure above~\ref{fig:input1} illustrates the basal insulin rates for all 20 patients over a 24-hour period.
...
\begin{figure}[!ht]
    \centering
    \includegraphics[width=1\linewidth]{dmmsrBasal.png} % replace with your image
    \caption{Basal insulin rates for all 10 children, 10 adolescence and 10 adult patients in DMMS.R over a 24-hour period. No immediate correlation is observable again, but age groups do show influence.}
    \label{fig:input2}
\end{figure}

The following figures below~\ref{fig:bolus_comparison} illustrate the correction boluses administered at breakfast, lunch, and dinner. Both sets of graphs demonstrate broadly similar trends; however, the differences between them reflect how each individual responds to bolus insulin. The second set of graphs is derived from the DMMS.R simulator, which includes data for children and adolescent patients. For our analysis, these younger age groups will not be simulated, and we will only compare ten patients from each simulator to ensure a fair comparison.
...
URL: \url{https://diabetes.org/about-diabetes/type-1}

\bibitem{treatment_type1}
Management and treatment. Cleveland Clinic.\\
URL: \url{https://my.clevelandclinic.org/health/diseases/21500-type-1-diabetes}

\bibitem{py_mgipsim}
py-mGIPsim: A Python-Based Metabolic Simulator.\\
URL: \url{https://illinoistech-itm.github.io/py-mgipsim/}

\bibitem{dmmsr}
DMMS.R: Diabetes Mellitus Metabolic Simulator in R.\\
URL: \url{https://tegvirginia.com/dmms-r/}
